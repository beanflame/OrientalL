/*
LPK 1.4.8
Losu Program-language Kits 1.4.8
...
class ls_vm
{
    public:
        ...
        string lsi(const char* _vm,const char* _func);
        ...
        void losu_debug_point(string _sign);
        void losu_debug_stack_scan();
        void losu_debug_var_scan();
        void losu_debug_space_scan(string _sign);
};

void ls_vm::losu_debug_point(string _sign)
{
    int tmp=0;
    vm_cpu_csip = 1;
    cout<<endl<<"\n----------------\n输入你要的操作:\n1.查看主栈内容  2.显示变量情况  3. 显示当前代码区间  4. 继续执行\n";
    do
    {
        cout<<"\n----------------\n";
        cin>>tmp;
        if(tmp==1) losu_debug_stack_scan();
        else if(tmp==2) losu_debug_var_scan();
        else if(tmp==3) losu_debug_space_scan(_sign);
        else if(tmp==4) break;
        else break;
    } while (tmp!=4);
    vm_cpu_csip =0;
...
void ls_vm::losu_debug_stack_scan()
{
    cout<<"栈编号\t栈内容\n";
    for(int i = 0;i< 17;i++)
    {
...
void ls_vm::losu_debug_var_scan()
{
    cout<<"变量名\t变量值\n";
    for(int i = 0;i< vaname.size();i++)
    {
        cout<<vaname[i]<<"\t"<<vavalue[i]<<endl;
    }
}
void ls_vm::losu_debug_space_scan(string _sign)
{
    cout<<"当前代码所属区间\t"<<_sign<<endl;
}
...
{
    typedef string (*LOSU_API_VOID)( const char* _s1,const char* _s2,const char* _s3,const char* _s4,const char* _s5,const char* _s6,const char* _s7,const char* _s8,const char* _s9,const char* _s10,const char* _s11,const char* _s12,const char* _s13,const char* _s14,const char* _s15,const char* _s16);
    ...
    const char* _dll = _str_dll.c_str();
        ...
        LOSU_API_VOID fun;
		fun = (LOSU_API_VOID)GetProcAddress(_ptr_dll,_func);
		_rt = fun( top().c_str(), stack[2].c_str(), stack[3].c_str(), stack[4].c_str(), stack[5].c_str(), stack[6].c_str(), stack[7].c_str(), stack[8].c_str(), stack[9].c_str(), stack[10].c_str(), stack[11].c_str(), stack[12].c_str(), stack[13].c_str(), stack[14].c_str(), stack[15].c_str(), stack[16].c_str());
        ...
        LOSU_API_VOID fun;
        fun = (LOSU_API_VOID)dlsym(_ptr_dll,_func);
        _rt = fun( top().c_str(), stack[2].c_str(), stack[3].c_str(), stack[4].c_str(), stack[5].c_str(), stack[6].c_str(), stack[7].c_str(), stack[8].c_str(), stack[9].c_str(), stack[10].c_str(), stack[11].c_str(), stack[12].c_str(), stack[13].c_str(), stack[14].c_str(), stack[15].c_str(), stack[16].c_str());
...
string ls_vm::lsi(const char* _vm,const char* _func)
{
    typedef string (*LOSU_API_VOID)( const char* _s1,const char* _s2,const char* _s3,const char* _s4,const char* _s5,const char* _s6,const char* _s7,const char* _s8,const char* _s9,const char* _s10,const char* _s11,const char* _s12,const char* _s13,const char* _s14,const char* _s15,const char* _s16);
    vector<LOSU_API_VOID> _fun;
    string _rt;
    string  _str_dll = LS_LSI_PATH;
	_str_dll = _str_dll + "losusi_" + _vm + ".lsd";
    const char* _dll = _str_dll.c_str();
    #if (LS_PLAT == 1)
        HMODULE _ptr_dll = LoadLibrary(_dll);
        if (_ptr_dll == NULL)
...
void read_src_file(string _filehead)
{
    ...
    ofstream _of;
    ifstream _if;
    _if.open(_filehead+".lsh",ios::in);
    _of.open(_filehead+".lsh.lpk",ios::out);
    if (_if.is_open()==0)
    {
        _if.close();
        _of.close();
...
int main(int argc,const char** argv)
{
    try
    {
        ...
        ofstream f;
        f.open((tmp+".lsh.lpk").c_str(),ios::out);
        f.close();
        f.open((tmp+".lsh.class.tmp.lpk").c_str(),ios::out);
        f.close();
        f.open((tmp+".lsh.obj.tmp.lpk").c_str(),ios::out);
        f.close();
        read_src_file(tmp);
        ...
        remove((tmp+".lsh.class.tmp.lpk").c_str());
        remove((tmp+".lsh.obj.tmp.lpk").c_str());
        remove((tmp+".lsh.lpk").c_str());
        remove((tmp+".lsh.lsi.lpk").c_str());
        remove(mainfile.c_str());
		//pthread_exit(NULL);

...
extern "C"
{
	...
	string losuvm_file_open(const char* filename,const char* filenum,const char* mod)
	{
	...
	string losuvm_file_print(const char* filenum,const char* str)
	{
		if (atoi(filenum) > 10000 || atoi(filenum) < 0)
		{
			throw "洛书运行时错误:	文件编号溢出\n";
	...
	string losuvm_file_input(const char* filenum)
	{
		string _tmp;
		if (atoi(filenum) > 10000 || atoi(filenum) < 0)
		{
	...
	string losuvm_file_lineinput(const char* filenum)
	{
		string _tmp;
		if (atoi(filenum) > 10000 || atoi(filenum) < 0)
		{
	...
	string losuvm_file_eof(const char* filenum)
	{
		if (atoi(filenum) > 10000 || atoi(filenum) < 0)
		{
			throw "洛书运行时错误:	文件编号溢出\n";
	...
	string losuvm_file_close(const char* filenum)
	{
		if (atoi(filenum) > 10000 || atoi(filenum) < 0)
		{
			throw "洛书运行时错误:	文件编号溢出\n";
